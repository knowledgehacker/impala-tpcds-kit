...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast ('2001-03-14' as date) - 30 days)
...
select  
 ...
 where
  ...
  ca_county in ('Yellowstone County','Montgomery County','Divide County','Cedar County','Manassas Park city') and
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 30992
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 25740
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 20311
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 21635
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 20532
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('1998-01-06' as date) and (cast('1998-01-06' as date) +  60 days)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('1998-03-08' as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1998-03-08' as date) - 30 days)
                and (cast ('1998-03-08' as date) + 30 days) 
 group by
    w_state,i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-04-17','1999-10-04','1999-11-10')))
